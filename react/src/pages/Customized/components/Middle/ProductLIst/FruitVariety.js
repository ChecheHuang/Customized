...
function FruitVariety(props) {
  ...
  useEffect(()=>{
    if(imageFront.length!==0){
      const newImageUrl=require(`../../../../../images/CustomizedPhotos/${imageFront}/${images}`).default
  ...
  const farmerImageUrl = "https://picsum.photos/500/400?random=" + avatar.replace(".jpg","");
  ...
  const newImageArray = imageArray&&imageArray.split(',').map((item)=>{
    const newImage=require(`../../../../../images/CustomizedPhotos/${imageFront}/${item}`).default
    return newImage
  })
